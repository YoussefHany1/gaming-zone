  ...
  return (
    <>
      <section className="flex flex-col justify-center items-center"> 
        <Link href='/latest/news' className="text-white p-5 px-10 bg-(--secondary) w-fit text-center rounded-2xl text-3xl font-bold">Latest News</Link>
        <div className="container py-10 flex flex-col">
          {items.slice(0, visibleCount).map((item, index) => {
            return (
              <div className={`news-item ${index} py-5 border-b-1 border-gray-600 hover:bg-(--secondary) duration-300 rounded-xl px-5 mx30`} key={index}>
                <a href={item.link} target="_blank" className="news-link text-white flex items-center justify-between sm:flex-row flex-col-reverse">
          ...
          {visibleCount < items.length && (
            <Link href='/latest/news' className="text-center text-white mt-10 text-2xl font-bold bg-(--secondary) px-10 py-2 cursor-pointer rounded-xl w-fit self-center hover:bg-(--primary) duration-300">See More</Link>
          )}
        </div>
      </section>
    </>
